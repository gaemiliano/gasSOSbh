...
// 1. CARREGAMENTO DINÂMICO DO MAPA (Resolve o erro da Vercel)
...
// Componente para lidar com lógica interna do Leaflet (Icones e Recentralização)
...
export default function GasSOS() {
  ...
  const [coords, setCoords] = useState<{lat: number, lng: number}>({ lat: -19.9167, lng: -43.9345 });
  const [isMounted, setIsMounted] = useState(false);

  // Garante que o mapa só carregue no navegador
  useEffect(() => {
    setIsMounted(true);
    if ('geolocation' in navigator) {
      navigator.geolocation.getCurrentPosition(
        (pos) => setCoords({ lat: pos.coords.latitude, lng: pos.coords.longitude }),
        (err) => console.log("Aguardando permissão de GPS...")
      );
    }
  }, []);
  ...
  const finalizarPedido = async () => {
    ...
    navigator.geolocation.getCurrentPosition(async (pos) => {
      ...
      alert("Ative o GPS para pedir socorro.");
      setLoading(false);
    });
  };
    ...
    return (
      <div className="min-h-screen bg-slate-50 p-8 flex flex-col items-center justify-center text-center font-sans">
        <div className="bg-white p-8 rounded-[40px] shadow-2xl w-full max-w-md border border-slate-100">
          ...
          <p className="text-slate-400 font-bold text-xs uppercase tracking-widest mt-2 mb-8">Pague o PIX para liberar o socorro</p>
          <div className="bg-slate-50 p-6 rounded-3xl border-2 border-dashed border-slate-200 mb-6 text-center">
            <p className="text-[10px] font-black text-slate-400 uppercase mb-4">Código PIX Copia e Cola</p>
            <div className="bg-white p-4 rounded-xl text-[10px] font-mono break-all text-slate-600 border border-slate-100 select-all">
          ...
          }} className="w-full bg-slate-900 text-white py-5 rounded-2xl font-black text-sm uppercase mb-4 shadow-xl active:scale-95 transition-all">Copiar Código PIX</button>
    ...
    <div className="min-h-screen bg-slate-900 flex flex-col font-sans text-slate-100 overflow-hidden">
      ...
      <div className="h-[45vh] w-full relative z-0 bg-slate-800">
        ...
        <div className="absolute top-6 left-6 z-[1000] bg-slate-900/80 backdrop-blur-md px-4 py-2 rounded-2xl border border-white/10 shadow-2xl">
           <h1 className="text-sm font-black tracking-tighter italic uppercase">GasSOS <span className="text-yellow-500 text-[10px] block">Belo Horizonte</span></h1>
        </div>
      </div>
      ...
      <div className="flex-1 bg-white rounded-t-[45px] -mt-10 relative z-10 p-8 flex flex-col shadow-[0_-20px_40px_rgba(0,0,0,0.5)]">
        ...
        <div className="space-y-6 flex-1">
          <div className="grid grid-cols-2 gap-4">
            <button onClick={() => setCombustivel('gasolina')} className={`p-6 rounded-[32px] border-4 transition-all ${combustivel === 'gasolina' ? 'border-yellow-400 bg-yellow-50' : 'border-slate-50 bg-slate-50 opacity-40'}`}>
              <span className="text-4xl block mb-2">⛽</span>
              <span className="font-black text-[10px] uppercase text-slate-500">Gasolina</span>
              <p className="font-black text-lg text-slate-900 italic">R$ {precos.gasolina},00</p>
            </button>
            <button onClick={() => setCombustivel('etanol')} className={`p-6 rounded-[32px] border-4 transition-all ${combustivel === 'etanol' ? 'border-green-500 bg-green-50' : 'border-slate-50 bg-slate-50 opacity-40'}`}>
              <span className="text-4xl block mb-2">🎋</span>
              <span className="font-black text-[10px] uppercase text-slate-500">Etanol</span>
              <p className="font-black text-lg text-slate-900 italic">R$ {precos.etanol},00</p>
            </button>
          </div>
          <div onClick={() => setPartidaFrio(!partidaFrio)} className={`p-4 rounded-2xl border-2 flex justify-between items-center transition-all ${partidaFrio ? 'border-blue-500 bg-blue-50 text-blue-700' : 'border-slate-100 bg-slate-50 text-slate-400'}`}>
            <div className="flex items-center gap-4">
              <span className="text-xl">❄️</span>
              <div className="text-left">
                <p className="font-black text-[10px] uppercase italic">Partida a Frio</p>
                <p className="text-[8px] font-bold opacity-60 uppercase">Essencial no Inverno</p>
              </div>
            </div>
        ...
        <button onClick={finalizarPedido} disabled={loading} className="w-full bg-slate-900 text-white mt-8 py-6 rounded-[24px] shadow-2xl font-black text-lg tracking-[0.2em] uppercase italic active:scale-95 transition-all flex justify-between px-10 items-center disabled:opacity-50">
