...
// 1. CARREGAMENTO DINÂMICO PARA NÃO QUEBRAR NO SERVIDOR
...
export default function GasSOS() {
  ...
  useEffect(() => {
    ...
    // 2. CONFIGURAÇÃO DO ÍCONE APENAS NO CLIENTE (MATA O ERRO WINDOW)
  ...
  const finalizarPedido = async () => {
    ...
    if (!error) setEtapa('pagamento');
    else alert("Erro ao enviar chamado.");
    setLoading(false);
  };

  // Se não montou ou ícone não carregou, exibe fundo preto para evitar erro de build
  if (!isMounted || !radarIcon) return <div className="min-h-screen bg-black" />;
      ...
      <div className="relative z-[1001] -mt-16 px-6 pb-12">
        ...
        {etapa === 'pedido' && (
          <div className="space-y-6 animate-in slide-in-from-bottom duration-700">
            ...
            <button onClick={() => setEtapa('dados')} className="w-full bg-yellow-500 text-black p-6 rounded-[30px] font-black uppercase italic shadow-[0_15px_40px_rgba(234,179,8,0.3)]">
        ...
        {etapa === 'dados' && (
          <div className="space-y-4 animate-in fade-in duration-500">
             <input placeholder="TEU NOME" value={nome} onChange={e => setNome(e.target.value)} className="w-full p-5 bg-black border-2 border-white/10 rounded-2xl outline-none focus:border-yellow-500 font-bold uppercase" />
             <input placeholder="PLACA DO VEÍCULO" value={placa} onChange={e => setPlaca(e.target.value)} className="w-full p-5 bg-black border-2 border-white/10 rounded-2xl outline-none focus:border-yellow-500 font-bold uppercase" />
             <input placeholder="WHATSAPP (DDD)" value={telefone} onChange={e => setTelefone(e.target.value)} className="w-full p-5 bg-black border-2 border-white/10 rounded-2xl outline-none focus:border-yellow-500 font-bold uppercase" />
        ...
        {etapa === 'pagamento' && (
          <div className="text-center space-y-6 animate-in zoom-in duration-500 pt-6">
            ...
            <div className="bg-white p-4 rounded-3xl text-black font-mono text-[10px] break-all">
                CHAVE_PIX_AQUI_00020126360014BR.GOV.BCB.PIX0114SUACHAVEAQUI5204000053039865405{total}.00
            </div>
